...
> **前瑞幸咖啡店长 × 零售全链路决策系统**
> 将零售经验转化为可解释规则——从选品评分到物流配发，由四阶段本地流水线完成演示
...
| 📖 案例库 | 3家虚拟业务案例 | 萌爪宠物/PawStyle/Bark&Co；公司、评价及效果数据均为演示设定 |
| 📈 选品评分 | 4维加权评分 + 区域市场 | 5大市场(北美/欧洲/东南亚/日韩/澳洲)，内置活动日历 |
| 💰 定价模型 | 成本拆解 + 7种货币换算 | 28%红线利润模型，使用内置演示汇率 |
| 🤖 销售自动化 | Scout→Price→Copy→Monitor | 本地规则与模板驱动的四阶段流水线 |
| 📦 库存监控 | 周转天数 + 安全库存 + 补货 | 分页表格，按当前数据执行规则预警 |
| 📤 商品上架 | 模拟上架 Shopify/Etsy/独立站 | 自动生成Listing并在本地保存上架记录，未调用真实平台 API |
